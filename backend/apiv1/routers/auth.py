...
from apiv1.schemas import UserIn, UserOut, ErrorOut
...
def authenticate(request, user: UserIn):
    ...
    usermodel = django.contrib.auth.authenticate(username=user.username, password=user.password)
...
def get_current_user(request):
    ...
    if request.user.is_authenticated:
        return UserOut(username=request.user.get_username(), user_id=str(request.user.pk))
    else:
