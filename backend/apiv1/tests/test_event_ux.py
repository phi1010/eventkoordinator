...
class EventUxPlaywrightTest(SnapshotMixin, ViteStaticLiveServerTestCase):
    ...
    def _login_via_navbar(self, page: Page, base_url: str) -> None:
        ...
        with page.expect_response(
            lambda response: (
                response.url.endswith("/api/v1/authenticate")
        ...
        page.get_by_role("link", name="Coordinator").wait_for(timeout=10_000)
    ...
    def _go_to_coordinator(self, page: Page, base_url: str) -> None:
        ...
        logger.debug("Clicking Coordinator nav link")
        page.get_by_role("link", name="Coordinator").click()
        try:
            page.wait_for_url("**/coordinator**", timeout=10_000)
        except PlaywrightTimeoutError:
            logger.warning("Coordinator link click did not change URL, falling back to direct /coordinator navigation")
            page.goto(base_url + "/coordinator")
            page.wait_for_url("**/coordinator**", timeout=10_000)

        logger.debug("Waiting for Series listbox in coordinator view")
        page.get_by_role("listbox", name="Series").wait_for(timeout=10_000)

    def _drag_in_calendar(self, page: Page, start_hour: int = 10, end_hour: int = 13, col_index: int = 1) -> None:
        ...
        logger.debug(
            ...
            col_index, x, y_start, start_hour, y_end, end_hour,
        )
        ...
        with sync_playwright() as playwright:
            ...
            try:
                with print_aria_on_timeout(page):
                    ...
                    with self.subTest(stage="select_series"):
                        logger.debug("Waiting for Series listbox to appear")
                        series_listbox = page.get_by_role("listbox", name="Series")
                        series_listbox.wait_for(timeout=10_000)
                        logger.debug("Waiting for series name %r inside listbox", self.series.name)
                    ...
                    with self.subTest(stage="select_event"):
                        logger.debug("Waiting for Events listbox to appear")
                        events_listbox = page.get_by_role("listbox", name="Events")
                        events_listbox.wait_for(timeout=10_000)
                        logger.debug("Waiting for event name %r inside listbox", self.event.name)
                    ...
                    with self.subTest(stage="edit_fields"):
                        logger.debug("Clearing and filling Name input: 'Updated Event Name'")
                    ...
                    with self.subTest(stage="drag_calendar"):
        ...
        with sync_playwright() as playwright:
            ...
            try:
                with print_aria_on_timeout(page):
                    ...
                    with self.subTest(stage="create_series"):
                        ...
                        create_series_btn.wait_for(timeout=10_000)
                    ...
                    with self.subTest(stage="edit_fields"):
                        logger.debug("Clearing and filling Name input: 'My New Workshop'")
                        name_input = page.get_by_label("Name")
                        name_input.clear()
